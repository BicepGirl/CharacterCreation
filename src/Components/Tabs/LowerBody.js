  ...
  return (
    <div className="">
      <div className="grid grid-cols-2">
        ...
        <button
          className=" text-orange-400 mt-4 text-right mr-4"
          onClick={() =>
            characterCustomizationOptionsBackButton(tempCompSelected)
          }
        >
          {`< Back`}{" "}
        </button>
      ...
      <div className="grid grid-cols-4 ">
        <div className="rounded-lg h-10 w-10 relative p-3  m-4 bg-[#606060] ">
          <button
            className="absolute "
            onClick={() => characterCustomizationOptions("", tabValue)}
          >
            <IonIcons.IoBanOutline color="silver " />
          </button>
        </div>
        {svgComponentData(color).lowerBody.map((val, index, color) => {
          // console.log('colorComp: ', val.fill)
          return (
            // <div className="rounded-lg h-10 w-10 relative p-3 m-4 bg-[#606060] ">
            <button
              onClick={() => characterCustomizationOptions(val, tabValue)}
              key={index}
            >
              {val}
            </button>
      ...
      <button
        className="rounded-lg bg-orange-600 m-4 p-2 pl-20 pr-20 text-white bottom-0 absolute hover:bg-gray-600 active:bg-gray-400"
        onClick={(e) => isApplyButton(e)}
        key={"applyButton"}
      >
        Apply
      </button>
